...
Run metadata
- Timestamp: 20260813T074941Z
- Repo root: `/workspaces/sparkta`
- Branch / implementation baseline: `issue-2-adopt-engineering-harness` / `13654afe3ec6f5f8aad909e129ebe680574352ac`
- Mode: static
- Commands executed: read-only Git context and static repository inspection
- Commands skipped: service boot, tests/builds, dependency installation, external calls, and deep history analysis
- Safety notes: no secrets read and no runtime/product/external state mutated
...
| Copilot skills | `.agents/skills/` | Exact allowlist: `eng-harness-flow`, `eng-harness-0-harnessability-assessment`, and `grill-agent-done` |
...
The canonical path is now `harness instructions` → doctor/help → focused checks → owned boot/readiness → work → full checks → stop. Harness checks delegate exactly to the root `justfile`. RPIV carries all five harness lifecycle seams. The committed engineering-harness skill surface is the exact three-name allowlist above; the package lock is provenance, not authorization. Test mechanisms include Fastify injection, an in-memory log sink, command spies, and detached process-group fixtures.
...
|---|---|---|
| `.harness/engineering-harness.md`, `.github/skills/README.md`, `justfile` | evidence | Canonical commands, evidence, the exact allowlist (`eng-harness-flow`, `eng-harness-0-harnessability-assessment`, and `grill-agent-done`), and lifecycle seams are governed |
