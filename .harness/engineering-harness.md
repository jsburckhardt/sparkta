...
The ambient CLI installation is owned by the configured development environment. Repository npm state owns only Sparkta dependencies; it does not install or reproduce the harness. Committed governance, extensions, skills, and discovery files make repository behavior portable. Engineering-harness validation checks non-empty `SKILL.md` files only for `eng-harness-flow`, `eng-harness-0-harnessability-assessment`, and `grill-agent-done`; unrelated sibling skills are ignored and preserved. `.harness/skills.lock.json` records source provenance rather than authorization. Do not run broad skill installation when it would restore excluded engineering-harness entries.
...
| Skill portability | Non-empty `SKILL.md` files for `eng-harness-flow`, `eng-harness-0-harnessability-assessment`, and `grill-agent-done`; unrelated siblings ignored and preserved | Committed skill content and the root named-skill guard; `.harness/skills.lock.json` is provenance only |
...
**L2 — Commands encoded.** Ambient harness 0.13.0 discovery, delegated focused/full checks, owned boot, dual readiness, safe stop, the three named engineering-harness skill checks, unrelated-sibling preservation, and managed commit guidance were exercised in the configured Node.js 24 environment. L3 is not claimed: sustained normal-work improvement-loop evidence has not been established.
