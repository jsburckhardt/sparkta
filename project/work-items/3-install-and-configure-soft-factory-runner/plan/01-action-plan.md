...
| AC-5  | T-2, T-4             | V-2, V-4                | Live operator documentation retains direct explicit-issue lifecycle commands and no duplicate checker guidance                                  |
...
2. **T-2 - Remove duplicate Soft Factory authority:** Keep `scripts/verify-soft-factory.mjs` deleted and remove every live or evidentiary reference to that checker or any equivalent static Runner configuration, compatibility, readiness, asset, manifest, or hash check. Use direct `soft-factory doctor --json` wherever such a verdict is required.
...
4. **T-4 - Reconcile documentation and evidence:** Align harness/skills discovery, Runner operations, architecture summaries, implementation evidence, verification evidence, and PR text with the corrected authority boundary. Mark superseded static-check and hash evidence invalid rather than carrying it forward.
