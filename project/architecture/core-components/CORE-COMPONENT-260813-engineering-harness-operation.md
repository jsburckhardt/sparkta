...
- Harness governance, extensions, cold-agent discovery guidance, and exactly these three engineering-harness GitHub Copilot skills MUST be committed under `.agents/skills/`: `eng-harness-flow`, `eng-harness-0-harnessability-assessment`, and `grill-agent-done`. No other `.agents/skills/` directory is allowed.
...
- `.agents/skills/` and `.github/skills/README.md` are the authoritative named allowlist surfaces. `.harness/skills.lock.json` records portable installation provenance only; it does not authorize additional skill names.
...
- Repository validation fails when an allowed skill is missing, an additional `.agents/skills/` directory exists, or live discovery documentation names an excluded skill.
...
The harness CLI is already implemented, installed, and configured as an ambient engineering tool, so duplicating installation in repository npm state would create an unnecessary and misleading ownership boundary. Committing governance, extensions, an explicit minimal skill allowlist, and discovery keeps repository behavior portable without importing unrelated packaged workflows; thin extensions preserve root command authority; and ownership-aware boot state prevents cleanup from terminating unrelated processes.
...
- Commit repository-local governance, extensions, only the three allowed engineering-harness skill directories, and matching discovery files; do not infer CLI installation ownership from them.
- Remove excluded skill directories and stale named references directly. Do not rerun a broad packaged installer that restores them.
