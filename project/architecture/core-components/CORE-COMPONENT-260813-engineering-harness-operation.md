...
This contract applies to validation of the already-installed ambient harness CLI in the configured development environment and to repository-local extensions, boot and readiness lifecycle, check delegation, engineering-harness GitHub Copilot skills, governance, agent discovery, RPIV injection seams, and commit guidance. The repository does not own or reproduce the ambient CLI installation, this contract permits separately governed cross-cutting skills such as the official Soft Factory skill, and it does not change the `.sparkta/apps/` and `.sparkta/runtime/` product-state boundary.
...
- Harness governance, extensions, cold-agent discovery guidance, and exactly these three engineering-harness GitHub Copilot skills MUST be committed under `.agents/skills/`: `eng-harness-flow`, `eng-harness-0-harnessability-assessment`, and `grill-agent-done`. No other engineering-harness skill is allowed; separately adopted cross-cutting components MAY authorize additional named skill directories, including the official `soft-factory` skill.
...
- `.agents/skills/` and `.github/skills/README.md` are the authoritative named skill inventory surfaces. This component authorizes exactly three engineering-harness names; each additional cross-cutting skill requires its own adopted component. `.harness/skills.lock.json` records engineering-harness installation provenance only and authorizes no skill name.
...
- A cold agent can discover the ambient tool boundary, every committed harness verb and briefing, and exactly the three allowed engineering-harness skills alongside any separately governed cross-cutting skill without global agent files or temporary skill sources.
- Repository validation fails when an approved skill is missing, an ungoverned `.agents/skills/` directory exists, or live discovery documentation names an excluded engineering-harness skill.
...
The harness CLI is already implemented, installed, and configured as an ambient engineering tool, so duplicating installation in repository npm state would create an unnecessary and misleading ownership boundary. Committing governance, extensions, an explicit minimal engineering-harness skill allowlist, and discovery keeps repository behavior portable without importing unrelated packaged workflows while permitting independently governed cross-cutting skills; thin extensions preserve root command authority; and ownership-aware boot state prevents cleanup from terminating unrelated processes.
...
- Commit repository-local governance, extensions, only the three allowed engineering-harness skill directories, and matching discovery files; allow additional named skills only when another adopted component governs them, and do not infer CLI installation ownership from any skill.
