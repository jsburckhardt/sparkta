...
Issue 7 manual Prototype 0 evidence is operated only with root `trial-init`, `trial-generate`, `trial-validate`, and `trials-check` recipes. Copied starter guidance requires app-bounded agents to implement the supplied interface prompt directly without parent-repository RPIV, harness, or Runner orchestration. Keep writes in its work-item `implementation/trials/` boundary; do not treat it as product lifecycle or access Runner-owned state.
