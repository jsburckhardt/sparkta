...
Issue 7 manual Prototype 0 evidence is operated only with root `trial-init`, `trial-generate`, `trial-validate`, and `trials-check` recipes. Keep writes in its work-item `implementation/trials/` boundary; do not treat it as product lifecycle or access Runner-owned state.
